...
def create_ass_file(word_boundaries, filename="subs.ass"):
    # PlayResX/Y definiują płótno 1080x1920
    # Alignment=5 to ABSOLUTNY ŚRODEK EKRANU
    # Fontname=Quicksand Bold (zainstalujemy ją w workflow)
    header = (
        "[Script Info]\nScriptType: v4.00+\nPlayResX: 1080\nPlayResY: 1920\nScaledBorderAndShadow: yes\n\n"
        "[V4+ Styles]\n"
        "Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding\n"
        "Style: Default,Quicksand Bold,90,&H0000FFFF,&H000000FF,&H00000000,&H00000000,-1,0,0,0,100,100,0,0,1,6,0,5,100,100,0,1\n\n"
        "[Events]\n"
    ...
    for i in range(0, len(word_boundaries), words_per_chunk):
        ...
        # Jeśli tekst ma więcej niż 12 znaków, wymuszamy łamanie linii dla efektu bubble
        if len(text) > 12:
            text = text.replace(" ", "\\N", 1)
            
        # Animacja pojawiania się ({\fad})
...
async def process_tts(text):
    ...
    try:
        # Jenny to najbardziej emocjonalny głos
        ...
        print(f"[TTS] Edge-TTS failed, using fallback...")
